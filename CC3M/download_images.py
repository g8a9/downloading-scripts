...
def process_row(idx, row):
    filepath = f"CC_IT_NEW/CC_IT_{idx:012d}.jpg"
...
if __name__ == "__main__":
    ...
    train_df = pd.read_csv(train_file, sep="\t", header=None, names=["caption", "url"])
    val_df = pd.read_csv(val_file, sep="\t", header=None, names=["caption", "url"])
    ...
    df["status"] = status
    df.to_csv("download_report.csv", index=None)
